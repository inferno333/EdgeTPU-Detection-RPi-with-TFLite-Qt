...
linux{
    contains(QMAKE_CXX, .*raspbian.*arm.*):{
        ...
        # Qt Raspberry Pi Sysroot
        SYSROOT_PATH = /home/javi/raspi/sysroot/

        # TensorFlow Lite lib path
        LIBS += -L$$PWD/../tensorflow/tensorflow/lite/tools/make/gen/rpi_armv7l/lib \
                -Wl,$$SYSROOT_PATH/usr/lib/arm-linux-gnueabihf/libunwind-arm.so.8 \
                -Wl,$$SYSROOT_PATH/usr/lib/arm-linux-gnueabihf/libc++abi.so.1 \
                -Wl,$$SYSROOT_PATH/usr/lib/arm-linux-gnueabihf/libedgetpu.so.1
